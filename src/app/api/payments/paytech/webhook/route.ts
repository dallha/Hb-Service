...
export async function POST(request: Request) {
  try {
    ...
    if (apiSecret) {
      const localSig = createHash('sha256').update(apiSecret).digest('hex');
      if (apiKeySha256 !== localSig) {
        console.error('[PayTech Webhook] Signature verification failed');
        return NextResponse.json({ error: 'Signature invalide' }, { status: 400 });
      }
    } else {
      console.warn('[PayTech Webhook] PAYTECH_API_SECRET is not set. Skipping signature verification.');
    }
